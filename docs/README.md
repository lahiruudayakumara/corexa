# Corexa Documentation

This is the primary documentation hub for Corexa.

## Documentation Map

### Architecture

- [Architecture Index](architecture/README.md)
- [Platform Architecture](architecture/corexa-platform-architecture.md)
- [Monorepo Structure](architecture/monorepo-structure.md)

### Product and Direction

- [Vision](product/vision.md)
- [Roadmap](product/roadmap.md)
- [Project Goals](project/goals.md)
- [Execution Plan](project/execution-plan.md)
- [GitHub Issues](project/github-issues.md)
- [Milestones](project/milestones.md)
- [Open Source Model](project/open-source-model.md)

### Development

- [Development Index](development/README.md)
- [Local Development](development/local-development.md)
- [Testing and Release](development/testing-and-release.md)

## Audience Guide

- new contributors should start with [CONTRIBUTING.md](../CONTRIBUTING.md)
- maintainers should review [GOVERNANCE.md](../GOVERNANCE.md)
- operators and security reviewers should read [SECURITY.md](../SECURITY.md)
- architects should start with [architecture/README.md](architecture/README.md)
